...
# Description: Defines various recommender models for the bachelor thesis baselines.
# Notes: Modified as part of the bachelor's thesis work. Random model was added.
# =============================================================================
...
class RandomRecommender:
    def __init__(self, item_idx, seed=42):
        self.item_idx = item_idx
    ...
    def predict_random(self, df, k=100):
        users = df.user_id.cat.categories.to_numpy()
        items = self.item_idx.to_numpy()
        X_test = get_sparse_matrix_from_dataframe(df, item_indices=self.item_idx)
        ...
        for user_idx, user in enumerate(users):
            ...
            seen_items = X_test[user_idx].toarray().flatten()
            unseen_items = items[seen_items == 0]
            
            sampled_items = np.random.choice(unseen_items, size=min(k, len(unseen_items)), replace=len(unseen_items) < k)
            
            user_ids.extend([user] * len(sampled_items))
        ...
        df = pd.DataFrame({
            "user_id": user_ids,
            "item_id": item_ids,
            "value": np.random.rand(len(user_ids))
        })
